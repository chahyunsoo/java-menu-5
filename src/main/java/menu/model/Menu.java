...
public enum Menu {
    ...
    ASIA_MENU("아시아", List.of("팟타이, 카오 팟, 나시고렝, 파인애플 볶음밥, 쌀국수, 똠얌꿍, 반미, 월남쌈, 분짜")),
    WEST_MENU("서양", List.of("라자냐, 그라탱, 뇨끼, 끼슈, 프렌치 토스트, 바게트, 스파게티, 피자, 파니니"));
    ...
    public static String pickMenu(String categoryName) {
        List<String> findMenus = List.of();
        for (Menu value : Menu.values()) {
            if (value.categoryName.equals(categoryName)) {
                findMenus = value.getMenus();
            }
        }
        return Randoms.shuffle(findMenus).get(0);
    }
}
